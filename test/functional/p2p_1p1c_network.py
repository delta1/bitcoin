...
class PackageRelayTest(BitcoinTestFramework):
    ...
    def raise_network_minfee(self):
        ...
        for node in self.nodes:
            assert_equal(node.getmempoolinfo()['minrelaytxfee'], FEERATE_MIN)
            assert_greater_than(node.getmempoolinfo()['mempoolminfee'], FEERATE_MIN)

    def create_basic_1p1c(self, wallet):
        low_fee_parent = wallet.create_self_transfer(fee_rate=FEERATE_MIN, confirmed_only=True)
    ...
    def create_package_2p1c(self, wallet):
        parent1 = wallet.create_self_transfer(fee_rate=FEERATE_MIN*10, confirmed_only=True)
        parent2 = wallet.create_self_transfer(fee_rate=FEERATE_MIN*20, confirmed_only=True)
