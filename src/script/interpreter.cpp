...
bool EvalScript(std::vector<std::vector<unsigned char> >& stack, const CScript& script, unsigned int flags, const BaseSignatureChecker& checker, SigVersion sigversion, ScriptExecutionData& execdata, uint64_t& varops_budget, ScriptError* serror)
{
    static const CScriptNum bnZero(0);
    static const CScriptNum bnOne(1);
    static const valtype vchFalse(0);
    ...
    // sigversion can only be TAPSCRIPT_V2 here
...
static bool ExecuteWitnessScript(const std::span<const valtype>& stack_span, const CScript& exec_script, unsigned int flags, SigVersion sigversion, const BaseSignatureChecker& checker, ScriptExecutionData& execdata, ScriptError* serror, std::optional<uint64_t> varops_budget)
{
    ...
    if (sigversion == SigVersion::TAPSCRIPT_V2) {
        ...
        if (!EvalScript(stack, exec_script, flags, checker, sigversion, execdata, *varops_budget, serror)) return false;
    } else {
...
static bool VerifyWitnessProgram(const CScriptWitness& witness, int witversion, const std::vector<unsigned char>& program, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror, bool is_p2sh, std::optional<uint64_t> varops_budget)
{
...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, const CScriptWitness* witness, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror, std::optional<uint64_t> varops_budget /* = std::nullopt */)
{
